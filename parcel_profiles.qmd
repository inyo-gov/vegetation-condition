...
build_parcel_notes <- function(parcel_id) {
  ...
  pgrass_pct <- if (!is.na(current$pGrass)) round(current$pGrass * 100, 2) else NA_real_
  ...
  dtw_text <- if (is.finite(dtw_current) && is.finite(dtw_baseline)) {
    paste0("Current DTW (ft BGS): ", round(dtw_current, 2),
           "; baseline avg (1984-86): ", round(dtw_baseline, 2), ".")
  } else {
...
for (i in seq_len(nrow(profile_parcels))) {
  ...
  p_prop <- ggplot(ts, aes(x = NominalYear, y = pGrass)) +
    ...
    scale_x_continuous(breaks = year_breaks, expand = expansion(mult = c(0, 0))) +
    scale_y_continuous(expand = expansion(mult = c(0.05, 0.28)), labels = function(x) sprintf("%.2f", x)) +
    coord_cartesian(xlim = x_limits, clip = "off") +
    theme_minimal(base_size = 11) +
    theme(
      plot.margin = plot_margin,
  ...
  if (nrow(dtw_ts) > 0 && any(!is.na(dtw_ts$DTW))) {
    dtw_stats <- dtw_ts %>%
      group_by(Year) %>%
      summarise(DTW = round(mean(DTW, na.rm = TRUE), 2), .groups = "drop") %>%
    ...
    p_dtw <- ggplot(dtw_stats, aes(x = Year, y = DTW)) +
      ...
      scale_y_reverse(labels = function(x) sprintf("%.2f", x)) +
