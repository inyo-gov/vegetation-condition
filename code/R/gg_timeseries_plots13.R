...
baseline_test <- function(baseline_vec, new_vec) {
  ...
  fit <- tryCatch({
    ...
    return(NA_real_)  # Return NA if the t-test fails
  })

  return(fit)
}

# Function to create bar plots with significance based on CI
plot_bar_discrete <- function(PID, df, column_name, baseline_years = 1984:1987, year_min = 1984, year_max = current_year, ylab_text = "Cover (%)", fill_color = "darkgreen", baseline_label = "Baseline") {
  ...
  stats$significant <- ifelse(!is.na(stats$CI_Lower) & !is.na(stats$CI_Upper) &
                                (stats$CI_Upper < baseline_avg | stats$CI_Lower > baseline_avg), TRUE, FALSE)
  stats$asterisk <- ifelse(stats$significant, "**", "")
...
plot_dtw_discrete <- function(PID, dtw_data, baseline_years = 1984:1986, year_min = 1984, year_max = current_year) {
...
create_ts_plots_pdf_gg_discrete <- function(PIDs, pdf_file = "Parcel_TimeSeries_CI_Significance.pdf", attributes, transects, dtw_data, rs_data, current_year = 2024) {
