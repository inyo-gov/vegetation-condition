...
targets <- c(
  "species",
  "n_parcels_sampled",
  "n_parcels_icwd_sampled",
  "n_parcels_ladwp_sampled",
  "ladwp_only_parcels",
  "cYear",
  "n_transects_sampled",
  "n_parcels_all_years",
...
In **`r cYear`**, ICWD measured **`r n_parcels_icwd_sampled`** reinventory vegetation parcels (Google Sheets) and LADWP measured **`r n_parcels_ladwp_sampled`** parcels, using **`r n_transects_sampled`** line-point intercept transects in total. LADWP delivers a long-format cover CSV and information file to ICWD each year. The merged master contains **`r n_parcels_sampled`** canonical reinventory parcel IDs because LADWP's extra parcel, **BIS082**, is included in the joint dataset even though ICWD did not field it. Statistical tests compared current-year vegetation cover to the **1984-1987** baseline for **Wellfield** parcels (pumping-affected) and **Control** parcels (no pumping).

LADWP parcel names **TIN067** and **UNW037** are alternate labels for single transects at **BLK006** (transect 1) and **MAN038** (transect 23), per the LADWP Information file (`Parcel2` / `Long_PCL` fields); they are not separate parcels. Beyond the ICWD sheet extract, LADWP also measured **`r paste(ladwp_only_parcels, collapse = ", ")`** (reinventory data included in the merged master).
...
pgrass_delta <- pgrass_current %>%
  ...
  mutate(
    pGrass_baseline = round(pGrass_baseline, 2),
    pGrass_current = round(pGrass_current, 2),
    pGrass_delta = round(pGrass_current - pGrass_baseline, 2)
  )
...
DT::datatable(
  ...
  DT::formatRound(
    columns = c("Cover_D", "Grass_D", "Shrub_D", "Cover", "Grass", "Shrub"),
    digits = 1
  ) %>%
  DT::formatRound(columns = c("DTW", "pGrass", "pShrub"), digits = 2) %>%
  DT::formatRound(columns = "DTW_pct", digits = 1)
```
...
### Parcel time series (PDF)

Downloadable stacked time-series plots (cover, DTW, NDVI, precipitation) for reinventory parcels: [Parcel_TimeSeries_2025.pdf](Parcel_TimeSeries_2025.pdf).
